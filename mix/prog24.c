...
int main(void)
{
	char health, home, sex;
	int age;

	puts("Enter health status[e/m/p] -->");
	health = getchar();
	puts("Enter home environment[c/v] -->");
	scanf("\n%c", &home);
	puts("Enter sex[m/f] -->");
	scanf("\n%c", &sex);
	puts("Enter age -->");
	...
	if (health == 'e' && (age >= 25 && age <=35) && home == 'c' && sex == 'm')
	{
		puts("Premium rate is Rs. 3 per thousand");
		puts("Maximum policy amount is Rs. 2 lakhs");

	}
	else if (health == 'e' && (age >= 25 && age <=35) && home == 'c' && sex == 'f')
	{
		puts("Premium rate is Rs. 4 per thousand");
		puts("Maximum policy amount is Rs. 1 lakhs");	
	}
	else if (health == 'p' && (age >= 25 && age <=35) && home == 'v' && sex == 'm')
	{
		puts("Premium rate is Rs. 6 per thousand");
		puts("Maximum policy amount is Rs. 10,000");	
	}
	else 
	{
		puts("You are not insured");
	}
